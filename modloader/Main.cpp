...
void GetAddresses()
{
	hOriginalBink = LoadLibrary("binkw32Hooked.dll");
	if (hOriginalBink)
	{
...
void OpenLogFile()
{
	...
	if (SHGetKnownFolderPath(FOLDERID_Documents, 0, 0, &docfolder) == S_OK)
	{
		...
		strcat_s(logfilepath, "\\thaw-modloader.log");
		fopen_s(&Log, logfilepath, "w");
		CoTaskMemFree(docfolder);
		return;
	}
	fopen_s(&Log, "thaw-modloader.log", "w");
}

DWORD WINAPI Start(LPVOID lpParam)
{
	OpenLogFile();
	logprintf("Tony Hawk's American Wasteland :: Modloader by xMike\n");
	GetAddresses();
	if (hOriginalBink)
	{
		logprintf("Loading addresses from original binkw32.dll succeeded!\n");
	}
	else
	{
		logprintf("Loading of original binkw32.dll addresses failed!\n");
		return 0;
	}
	SetExecutableFolder();
	loadPlugins();
